...
function pushModalHistory_(modalId) {
  // Mantenido para compatibilidad con invocadores existentes sin inflar el historial
}
...
function initAndroidBackHistory_() {
  // Trampa de historial inicial (freno de mano para Android)
  try {
  ...
  window.addEventListener('popstate', () => {
    ...
    // 1. SI LA PANTALLA ACTUAL ES MICRO_HOME
    if (current === 'MICRO_HOME') {
      const now = Date.now();

      // Si el usuario presionó Back dos veces seguidas en menos de 2000 ms estando en Home
      ...
      // Primer toque en Home (o pasaron más de 2000 ms desde el toque anterior en Home)
    ...
    // 2. SI LA PANTALLA ACTUAL ES CUALQUIER OTRA (NAVEGACIÓN INTERNA)
    // BLANQUEA OBLIGATORIAMENTE LA VARIABLE para que ningún toque previo contamine la salida
    lastBackPressTime = 0;

    // Re-arma el freno de mano inmediatamente
    history.pushState({ guard: true }, '');

    // Ejecuta la acción de retroceso de UI correspondiente a esa pantalla
    switch (current) {
      case 'MODAL':
        closeAllActiveModalsWithoutSaving_();
        break;
      ...
      case 'FAB_MENU':
        if (typeof toggleMacroFabMenu === 'function') toggleMacroFabMenu(false, false);
        else {
          const fabMenu = document.getElementById('macro-fab-menu');
          if (fabMenu) fabMenu.classList.add('hidden');
        }
      ...
      case 'MICRO_AUDIT':
        toggleBudgetAuditModal(false);
        break;

      case 'MICRO_FUTURE_DAYS':
        toggleFutureDaysModal(false);
        break;
      ...
      case 'MACRO_VALUE_EDIT':
        toggleValueEditModal(false);
        break;

      case 'MACRO_SERVICE_EDIT':
        toggleServiceEditModal(false);
        break;
      ...
      case 'MACRO_SALARY_ADJUST':
        toggleSalaryAdjustModal(false);
        break;

      case 'MACRO_FIXED_EXPENSE':
        toggleFixedExpenseModal(false);
        break;
      ...
      case 'MACRO_TOTAL_EXPENSES':
        setResumenInnerTab('incomes', false);
        break;

      case 'MACRO_HOME':
        switchView('micro', false);
        break;
    }
  });
